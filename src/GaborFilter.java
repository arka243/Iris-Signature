...
public class GaborFilter {
    ...
    public static FeatureVector[] filter(float image[], float mask[], int width, float scales[])
    {
        int nrOrders = 3;
        FeatureVector ls[][][] = new FeatureVector[scales.length][nrOrders][];
        ...
        FeatureVector Ln[] = new FeatureVector[length];
        int index = 0;
        for(int j = 0; j < scales.length; j++)
        {
            for(int order = 0; order < nrOrders; order++)
    ...
    public static FeatureVector[] L(float image[], float mask[], int width, int n, double scale, float theta[])
    {
        FeatureVector f[] = new FeatureVector[theta.length];
        ...
        if(n == 0)
        {
            ...
            f[0] = new FeatureVector(name(n, scale, 0.0D, ""), L[0]);
        } else
        if(n == 1)
        {
            volume.Kernel1D k0 = new GaussianDerivative(scale, 0);
            ...
            for(int i = 0; i < theta.length; i++)
            {
                ...
                f[i] = new FeatureVector(name(n, scale, theta[i], ""), L[i]);
            }
